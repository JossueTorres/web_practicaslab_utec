...
if ( isset($_GET["est"]) ) {
	$dbmaq_estado_maquina = $_GET['est'];
}else{
	$dbmaq_estado_maquina = 'I';
}
...
if ( $dbmaq_codlab > 0  && $dbmaq_fila > 0 && $dbmaq_columna > 0 ){
	...
	$sql = "UPDATE `adm_maq_maquinas` SET `maq_estado_maquina`='$dbmaq_estado_maquina' where maq_codlab = $dbmaq_codlab and maq_fila = $dbmaq_fila and maq_columna = $dbmaq_columna";
	$sql2 ="INSERT INTO `practicas_lab`.`adm_rac_registro_actividad` ( `rac_codlab`, `rac_fila`, `rac_columna`, `rac_fecha`,`rac_tipo`) VALUES ( $dbmaq_codlab, $dbmaq_fila, $dbmaq_columna, '$fechaServer','$dbmaq_estado_maquina'); ";
